...
import com.github.tarsys.android.kotlin.orm2.toFilter
import com.github.tarsys.android.kotlin.orm2.toInt
...
inline fun <reified T:IOrmEntity>KClass<T>.mapSoftEntity(cursorEntity: Cursor): T?{
    try{
        ...
        Log.e("mapSoftEntity", ex.message.toNotNullString())
        ex.printStackTrace()
    }
...
inline fun <reified T:IOrmEntity> KClass<T>.filterMap(fullMap: Boolean, predicate: (T) -> Boolean): ArrayList<T>{
    ...
    val sqliteDb = SgbdEngine.databaseSQLite(true)
...
fun <T: IOrmEntity>T.delete(): Boolean{
    ...
    val sqliteDb = SgbdEngine.databaseSQLite(false)
...
private val <T: IOrmEntity>T.hasFieldEntities: Boolean
    get() = this.javaClass.kotlin.entityFields.isNotEmpty() || this.javaClass.kotlin.entityListFields.isNotEmpty()
        ...
        for(field in entityFields){
            try{
                ...
                if (field.tableField!!.DataType == DBDataType.EntityDataType && value is IOrmEntity){
                    ...
                    returnValue += value as ArrayList<IOrmEntity>
                }
            }catch (ex: Exception){
                returnValue.clear()
            }
    ...
    try{
        ...
        if (dbEntity != null){
            ...
            for (field in classFields){
                whereClause += if (whereClause.isNotEmpty()) " and " else String().Empty
                whereClause += (if (forTableRelation) "${dbEntity.TableName.toLowerCase(Locale.ROOT)}_" else String().Empty) +
                        "${if(field.tableField!!.DataType == DBDataType.DateDataType) SQLiteSupport.PREFIX_DATE_FIELD else String().Empty}${field.tableField!!.FieldName}" +
                        "=?"
                ...
                when (field.tableField!!.DataType){
                    ...
                    DBDataType.RealDataType -> whereData.add((fieldValue as? Float ?: field.tableField!!.DefaultValue.toFloatOrNull()?: 0f).toString().replace(",","."))
        ...
        Log.e("primaryKeyFilter", ex.message.toNotNullString())
        ex.printStackTrace()
    }
...
private val <T: IOrmEntity>T.foreignKeyContentValues: ContentValues?
    get(){
        ...
        try{
            ...
            if (dbEntity != null){
                ...
                for (field in primaryKeyProperties){
                    ...
                    when(field.tableField!!.DataType){
                        ...
                        DBDataType.DateDataType -> returnValue.put(fieldName, (fieldValue as? Date)?.time?.toString() ?: field.tableField!!.DefaultValue
                        )
                    }
                }
            }
            ...
            returnValue = null
            Log.e("foreignKeyContentValues", ex.message.toNotNullString())
            ex.printStackTrace()
        }
...
private fun <T: IOrmEntity>T.findEntityByPrimaryKey(sqliteDb: SQLiteDatabase?, recursiveLoad: Boolean): Boolean {
    try{
                                ...
                                field.setter.call(this, when(tableField!!.DataType){
                                    ...
                                    DBDataType.EntityListDataType ->{
                                        ...
                                        if (fieldDbEntity != null && (field.returnType.javaClass.isArray || field.returnType.isSubtypeOf(ArrayList::class.starProjectedType)) && tableField.EntityClass.isSubclassOf(IOrmEntity::class)){
                                            ...
                                            val relationTable = "rel_${dbEntity.TableName.toLowerCase(Locale.ROOT)}_${fieldDbEntity.TableName.toLowerCase(Locale.ROOT)}"
                    ...
                    this.javaClass.kotlin.primaryKeyProperties.forEach { pkf ->
                        ...
                        if (pkf is KMutableProperty){
                            pkf.setter.call(this, when (pkTableField.DataType){
                                ...
                                DBDataType.StringDataType -> String().Empty
        ...
        Log.e("findEntityByPrimaryKey", ex.message.toNotNullString())
        ex.printStackTrace()
    }
...
private fun <T: IOrmEntity>T.delete(sqliteDb: SQLiteDatabase): Boolean{
    try{
        ...
        Log.e("IOrmEntity.delete", ex.message.toNotNullString())
        ex.printStackTrace()
    }
...
private fun <T: IOrmEntity>T.save(sqliteDb: SQLiteDatabase, forceEntitySave: Boolean): Boolean{
    try{
        ...
        if (sqliteDb.isOpen && dbEntity != null){
            ...
            if (pkFilter != null){
                ...
                fields.forEach { field ->
                    try{
                        ...
                        when (field.dbDataType){
                            ...
                            DBDataType.RealDataType -> contentValues.put(field.fieldName, fieldValue as? Float ?: field.tableField!!.DefaultValue.toFloat()
                            )
                            DBDataType.IntegerDataType, DBDataType.LongDataType -> contentValues.put(field.fieldName,
                                fieldValue as? Long ?: (fieldValue as? Int)?.toLong() ?: try{ field.tableField!!.DefaultValue?.toLong()}catch (ex:Exception){ 0L }
                            )
                            ...
                            DBDataType.EntityListDataType -> {
                                try {
                                    ...
                                    Log.e("IOrmEntity.save::EList", ex.message.toNotNullString())
                                    ex.printStackTrace()
                                }
                            }
                            DBDataType.EnumDataType -> {
                        ...
                        Log.e("IOrmEntity.save->map", ex.message.toNotNullString())
                        ex.printStackTrace()
                        error = true
                        return@forEach
                    }
                ...
                try{
                    ...
                    Log.e("IOrmEntity.save", "Innexistent Entity: ${ex.message.toNotNullString()}")
                    ex.printStackTrace()
                }
                ...
                if (filterRelations != null){
                    entityListTableFields.forEach { x ->
                        ...
                        val relatedTable = "rel_${dbEntity.TableName.toLowerCase(Locale.ROOT)}_$entityTableName"
                        ...
                        while (queryEntity?.moveToNext() == true){
                            var fgFilter = String().Empty
                            val fgValues: ArrayList<String> = arrayListOf()

                            queryEntity.columnNames.filter { xc -> xc.startsWith(entityTableName) }.forEach { column ->
                                fgFilter += "${if(fgFilter.isEmpty()) String().Empty else " and "}${column.replace("${entityTableName.toLowerCase(Locale.ROOT)}_", String().Empty)} = ?"
                                fgValues += queryEntity.getInt(queryEntity.getColumnIndex(column)).toString()
                            }
                            if (fgFilter.isNotEmpty() && fgValues.isNotEmpty()) sqliteDb.delete(entityTableName, fgFilter, fgValues.toTypedArray())
                        }
                        queryEntity?.close()
                    ...
                    entityListFields.forEach { eField ->
                        val relatedTable = "rel_${dbEntity.TableName.toLowerCase(Locale.ROOT)}_${eField.javaClass.kotlin.tableName}"
                        ...
                        if (returnValue){
                            ...
                            if (deleteFilter != null){
                                try{
                                    ...
                                    Log.e("IOrmEntity.save:X", ex.message.toNotNullString())
                                    ex.printStackTrace()
                                }
                            }
                            try{
                                ...
                                Log.e("IOrmEntity.save:Y", ex.message.toNotNullString())
                                ex.printStackTrace()
                            }
                        }
                    }
        ...
        Log.e("IOrmEntity.save", ex.message.toNotNullString())
        ex.printStackTrace()
    }
    return false
}
